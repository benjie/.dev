...
the query response from this normalized store. For example, imagine our initial
profile fetch was:
...
Our very first entity is the request itself; we can identify the request using
its operation type, operation name, and the values of the variables. For our
example above, we might use an identifier such as:
`query.ProfileFetch|{id:'U9DB7'}`.

The response has one root key, `user`, and we can see from the data inside of
`user` that it is an entity (it has an `id`, which is its cache identifier), so
we can replace it with a reference. But it's also critical to notice that the
`user` field accepts arguments, and since the arguments can change the
result of 

There are many ways of building a
normalized store, but ours will leverage the fact that a response key in
GraphQL can never contain a `$` symbol to use an object with a `$ref` key to
...
set the `id` of a `User` record to `query.ProfileFetch|{id:'U9DB7'}` which
would cause issues with the design here. This is easy to solve by either
prepending all identifiers (except the operation identifiers you build
yourself) with a specific symbol e.g. `@` or `$`, or by storing the top level
GraphQL requests into their own store.
...
Note in particular the highlit line, since the `posts` field accepts

Continuing like this

The resulting normalized cache you'll notice is much flatter than the response data:
