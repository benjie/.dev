...
description:
  ...
  GraphQL API via an allowlist, without having a name that's easily confusible
...
At app build time, extract the GraphQL documents (queries, etc) and make them
available to the server. At run time, send `documentId` instead of the whole
document; only accept requests with a `documentId`.
...
a **trusted document** is any GraphQL document the server can trust, typically
